...
from app.services.batch_archives import build_batch_zip
...
def job_batch_download_zip(
    batch_id: int,
    download_token: str | None = Query(default=None, pattern=r'^[a-f0-9]{32}$'),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    try:
        batch = db.query(JobBatch).filter(JobBatch.id == batch_id, JobBatch.user_id == current_user.id).first()
        if not batch:
            raise HTTPException(404, '批次不存在或无权下载。')
        rows = db.query(Job, JobFile).join(JobFile, JobFile.job_id == Job.id).filter(
            Job.batch_id == batch.id, Job.user_id == current_user.id,
            Job.status == 'completed', latest_attempt_filter(), JobFile.file_type == 'output_video',
        ).order_by(Job.batch_index.asc().nullslast(), Job.id, JobFile.id.desc()).all()
        files, seen, used_names = [], set(), set()
        for job, output in rows:
            source = Path(output.file_path)
            if job.id in seen or not source.is_file():
                continue
            seen.add(job.id)
            files.append((source, _unique_zip_name(job_video_download_filename(db, job), used_names)))
        response = build_batch_zip(files, _batch_zip_filename(batch))
        result = 'ready'
        ...
        result = 'error'
    if download_token:
        # Nonce reports attachment readiness, never authentication or transfer completion.
        response.set_cookie('batch_zip_' + download_token, result, max_age=300,
                            path='/app/job-batches', samesite='strict')
    return response
