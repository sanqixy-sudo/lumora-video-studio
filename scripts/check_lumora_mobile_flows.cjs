...
const base='http://127.0.0.1:8100',out=path.resolve('runtime/mobile_review_final');
fs.mkdirSync(out,{recursive:true});
(async()=>{
const browser=await chromium.launch({headless:true});const context=await browser.newContext({viewport:{width:390,height:844},hasTouch:true,isMobile:true,reducedMotion:'reduce'});const page=await context.newPage();page.on('dialog',dialog=>dialog.accept());const errors=[],checks=[];page.on('pageerror',e=>errors.push(e.message));
...
await record('表格菜单不被裁切、缩小屏幕后移动导航可关闭',async()=>{
 await page.goto(base+'/admin/jobs/page');await page.locator('.wb-row-trigger').first().scrollIntoViewIfNeeded();await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));await page.locator('.wb-row-trigger').first().click();const menu=page.locator('body>.wb-menu-open');assert.equal(await menu.count(),1);const box=await menu.boundingBox();assert.ok(box.x>=0&&box.x+box.width<=390&&box.y+box.height<=1000);await page.keyboard.press('Escape');
...
await record('手机生成、导入、渠道切换、登录注册失败恢复及成功登录',async()=>{
await page.setViewportSize({width:390,height:844});await page.goto(base+'/app?scenario=error');
...
await page.locator('.wb-row-trigger').first().scrollIntoViewIfNeeded();await page.evaluate(()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve))));await page.locator('.wb-row-trigger').first().click();assert.equal(await page.locator('body>.wb-menu-open').count(),1);await page.keyboard.press('Escape');
